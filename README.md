...
| `labs/lab3_s3_public_check/` | **Lab 3 – Auto‑Detect Public S3 Buckets** | GitHub Actions workflow runs `s3_public_check.py` in the **Production** account, scans all S3 buckets for public access, publishes Security Hub findings, and writes JSON summary evidence to a centralized S3 evidence bucket in the **Management/General** account. Implements A.9.4.1 (Information Access Restriction). |
